	                ...
	                execute(function (resp) {
	                    $scope.$apply(function () {
	                        ...
	                        if (resp.error) {
	                            ...
	                            $scope.alertStatus = 'warning';
	                            $log.error($scope.messages + ' Field : ' + JSON.stringify($scope.conference));
	                            ...
	                            // The request has succeeded.
	                            $scope.messages = 'A New Field Profile has been created : ' + resp.result.name;
	                            $scope.alertStatus = 'success';
	                            $scope.submitted = false;
	                            $scope.conference = {};
	                            $log.info($scope.messages + ' : ' + JSON.stringify(resp.result));
...
conferenceApp.controllers.controller('ShowConferenceCtrl', function ($scope, $log, oauth2Provider, HTTP_ERRORS) {
    ...
    /**
     * Holds the state if off canvas is enabled.
     *
     * @type {boolean}
     */
    ...
    /**
     * Sets the selected tab to 'ALL' Just implemented this experimentally Not required for our project.
            ...
            execute(function (resp) {
                $scope.$apply(function () {
                    ...
                    if (resp.error) {
                        // The request has failed.
                        var errorMessage = resp.error.message || '';
                        $scope.messages = 'Failed to query the Fields created : ' + errorMessage;
                        $scope.alertStatus = 'warning';
                        ...
                        // The request has succeeded.
                        $scope.submitted = false;
                        $scope.messages = 'Query succeeded : Fields you have created';
                        $scope.alertStatus = 'success';
                        $log.info($scope.messages);
                        $scope.conferences = [];
                        angular.forEach(resp.items, function (conference) {
        ...
        gapi.client.conference.getConference({
            ...
            $scope.$apply(function () {
                ...
                if (resp.error) {
                    ...
                    $scope.conference.requiredSensorData=resp.result.requiredSensorData;
                    $scope.conference.actualSensorData=resp.result.actualSensorData;
                }
            });
        });
