...
void Daemon::UpdateDeviceState() {
  ...
  int32_t moisture_reading;
  std::string moisture_reading_string;
  if (!led_service_->GetAllLEDs(&leds, nullptr))
    return;

  if (!led_service_->GetMoistureReading(&moisture_reading, nullptr))
    return;

  moisture_reading_string = std::to_string(moisture_reading);
  int32_t dummy = 555;
  //std::string dummy_s = std::to_string(dummy);

  brillo::VariantDictionary state_change{
    {"_ledflasher._status", status_},
    {"_ledflasher._leds", leds},
    {"_ledflasher._moisture",moisture_reading_string}
  };
